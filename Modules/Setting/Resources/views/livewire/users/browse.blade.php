<div>
    ...
    <x-breadcrumb>
        ...
        <li class="flex">
            <div class="flex items-center">
                ...
                <span aria-current="page" class="ml-4 text-sm font-medium text-gray-500">{{ __('Administrators & roles') }}</span>
            </div>
        </li>
    </x-breadcrumb>
    ...
    <div class="px-4 sm:px-6 lg:px-8 divide-y divide-gray-200">
        <div class="py-10">
            <div class="flex items-center">
            ...
            <div class="mt-8 grid gap-4 sm:grid-cols-2 sm:gap-6 md:grid-cols-3 md:gap-8">
                @foreach($roles as $role)
                    <a href="#" class="group flex flex-col justify-between p-4 border border-gray-200 hover:bg-gray-50 rounded-md overflow-hidden focus:border-gray-300 focus:outline-none transition duration-200 ease-in-out">
                        <div class="flex items-center justify-between">
                            <span class="text-xs leading-4 text-gray-400 font-semibold uppercase tracking-wider">{{ $role->users->count() }} {{ \Illuminate\Support\Str::plural(__("Account"), $role->users->count()) }}</span>
                        ...
                        <div>
                            ...
                            <p class="mt-1 flex items-center text-sm text-purple-600 group-hover:text-purple-500 transition duration-200 ease-in-out">
                                {{ __("View details") }}
                                <span class="ml-2">
        ...
        <div class="py-10">
            <div class="pb-6 space-y-3 sm:flex sm:items-center sm:justify-between sm:space-x-4 sm:space-y-0">
                ...
                <div>
                    <a href="{{ route('admin.settings.users.new') }}" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-purple-600 hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500">
                            ...
                            <tbody class="bg-white divide-y divide-gray-100" x-max="1">
                                @foreach($users as $user)
                                    <tr>
                                        ...
                                        <td class="hidden md:table-cell px-6 py-3 whitespace-no-wrap text-sm leading-5 text-gray-500 text-right">
                                            {{ $user->hasRole(config('modules.setting.users.admin_role')) ? __("Full") : __("Limited") }}
                                        </td>
    ...
    <x-modal wire:model="displayModal" maxWidth="2xl">
        <div class="bg-white">
        ...
        <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
            <span class="flex w-full rounded-md shadow-sm sm:ml-3 sm:w-auto">
                <x-button wire:click="addNewRole" type="button" wire:loading.attr="disabled">
            ...
            <span class="mt-3 flex w-full rounded-md shadow-sm sm:mt-0 sm:w-auto">
                <x-cancel-button wire:click="cancel" type="button">
                    {{ __('Cancel') }}
                </x-cancel-button>
            </span>
        </div>
