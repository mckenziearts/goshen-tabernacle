    ...
    <form wire:submit.prevent="save">
        ...
        <div class="relative p-4 sm:px-5">
            <div class="space-y-4 sm:space-y-5">
                <x-forms.group label="Avatar" for="avatar" :error="$errors->first('avatar')" noShadow>
                    <x-forms.avatar-upload wire:model.defer="avatar" id="avatar">
                        <span class="h-12 w-12 rounded-full overflow-hidden bg-secondary-100 dark:bg-secondary-700 flex items-center justify-center">
                            @if ($avatar)
                                <img class="h-12 w-12 object-cover" src="{{ $avatar->temporaryUrl() }}" alt="Author avatar">
                            @elseif($previewImage)
                                <img class="h-12 w-12 object-cover" src="{{ $previewImage }}" alt="Author avatar">
                            @else
                ...
                <x-forms.group label="Nom" for="name" :error="$errors->first('name')" isRequired>
                    <x-forms.input wire:model.defer="name" type="text" id="name" placeholder="Nom complet de l'artiste" autocomplete="off" />
                </x-forms.group>
            </div>
        </div>
