    ...
    <x-breadcrumb>
        ...
        <li class="flex">
            <div class="flex items-center">
                ...
                <span aria-current="page" class="ml-4 text-sm font-medium text-secondary-500">{{ __('Update event') }}</span>
            </div>
        </li>
    </x-breadcrumb>
                ...
                <div class="ml-6">
                    ...
                    <div class="mt-6 grid gap-4 sm:grid-cols-2 sm:gap-5">
                        ...
                        <x-forms.group wire:ignore class="sm:col-span-2" label="Event Description" for="description" :error="$errors->first('description')">
                            <livewire:dashboard::forms.trix :value="$description" />
                        </x-forms.group>
                    </div>
                </div>
            </div>
                            ...
                            <div class="mt-1 bg-white rounded-md shadow-sm -space-y-px" x-ref="radiogroup">
                                ...
                                <div :class="{ 'border-secondary-200': !(active === 0), 'bg-primary-50 border-primary-200 z-10': active === 0 }" class="relative border rounded-tl-md rounded-tr-md p-4 flex bg-primary-50 border-primary-200 z-10">
                                    <div class="flex items-center h-5">
                                        <input wire:model="privacy" id="settings-option-0" name="privacy_setting" type="radio" @click="select(0)" @keydown.space="select(0)" @keydown.arrow-up="onArrowUp(0)" @keydown.arrow-down="onArrowDown(0)" class="h-4 w-4 text-primary-600 cursor-pointer focus:ring-primary-500 border-secondary-300" value="public">
                                    </div>
                                ...
                                <div :class="{ 'border-secondary-200': !(active === 1), 'bg-primary-50 border-primary-200 z-10': active === 1 }" class="relative border p-4 flex border-secondary-200">
                                    <div class="flex items-center h-5">
                                        <input wire:model="privacy" id="settings-option-1" name="privacy_setting" type="radio" @click="select(1)" @keydown.space="select(1)" @keydown.arrow-up="onArrowUp(1)" @keydown.arrow-down="onArrowDown(1)" class="h-4 w-4 text-primary-600 cursor-pointer focus:ring-primary-500 border-secondary-300" value="private">
                                    </div>
                                ...
                                <div :class="{ 'border-secondary-200': !(active === 2), 'bg-primary-50 border-primary-200 z-10': active === 2 }" class="relative border rounded-bl-md rounded-br-md p-4 flex border-secondary-200">
                                    <div class="flex items-center h-5">
                                        <input wire:model="privacy" id="settings-option-2" name="privacy_setting" type="radio" @click="select(2)" @keydown.space="select(2)" @keydown.arrow-up="onArrowUp(2)" @keydown.arrow-down="onArrowDown(2)" class="h-4 w-4 text-primary-600 cursor-pointer focus:ring-primary-500 border-secondary-300" value="invitation">
                                    </div>
