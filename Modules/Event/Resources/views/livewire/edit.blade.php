...
<div>
    ...
    <x-breadcrumb>
        <li class="flex">
            <div class="flex items-center">
                <svg class="flex-shrink-0 w-6 h-full text-secondary-200" viewBox="0 0 24 44" preserveAspectRatio="none" fill="currentColor" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                    <path d="M.293 0l22 22-22 22h1.414l22-22-22-22H.293z" />
                </svg>
                <a href="{{ route('admin.events') }}" class="ml-4 text-sm font-medium text-secondary-500 hover:text-secondary-700">{{ __('Events') }}</a>
            </div>
        </li>
        <li class="flex">
            <div class="flex items-center">
                <svg class="flex-shrink-0 w-6 h-full text-secondary-200" preserverAspectRatio="none" viewBox="0 0 24 44" fill="currentColor" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                    <path d="M.293 0l22 22-22 22h1.414l22-22-22-22H.293z" />
                </svg>
                <span aria-current="page" class="ml-4 text-sm font-medium text-secondary-500">{{ __('Modifier l\'event') }}</span>
            </div>
        </li>
    </x-breadcrumb>

    <x-page-header class="bg-secondary-50">
    ...
    <main class="max-w-2xl mx-auto px-4 lg:pb-16 divide-y divide-secondary-200">
                ...
                <div class="ml-6">
                    ...
                    <div class="mt-6 grid gap-4 sm:grid-cols-2 sm:gap-5">
                        ...
                        <x-forms.group wire:ignore class="sm:col-span-2" label="Event Description" for="description" :error="$errors->first('description')">
                            <x-trix name="description" wire:model="description" styling="block w-full rounded-md shadow-sm border-secondary-300 focus:border-purple-300 focus:ring focus:ring-purple-200 focus:ring-opacity-50 sm:text-sm">
                                {{ $description }}
                            </x-trix>
                        </x-forms.group>
                    </div>
            ...
            <div class="flex">
                ...
                <div x-data="{ on: @entangle('is_visible') }" class="ml-6 flex-grow">
                    <div class="flex-grow flex items-center justify-between">
                        ...
                        <button type="button" class="relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 bg-secondary-200" aria-pressed="false" x-ref="switch" x-state:on="Enabled" x-state:off="Not Enabled" :class="{ 'bg-purple-600': on, 'bg-secondary-200': !(on) }" aria-labelledby="availability-label" :aria-pressed="on.toString()" @click="on = !on">
                    ...
                    <div class="mt-6">
                        <fieldset x-data="radioGroup()">
                            ...
                            <div class="mt-1 bg-white rounded-md shadow-sm -space-y-px" x-ref="radiogroup">
                                ...
                                <div :class="{ 'border-secondary-200': !(active === 0), 'bg-purple-50 border-purple-200 z-10': active === 0 }" class="relative border rounded-tl-md rounded-tr-md p-4 flex bg-purple-50 border-purple-200 z-10">
                                    <div class="flex items-center h-5">
                                        <input wire:model="privacy" id="settings-option-0" name="privacy_setting" type="radio" @click="select(0)" @keydown.space="select(0)" @keydown.arrow-up="onArrowUp(0)" @keydown.arrow-down="onArrowDown(0)" class="h-4 w-4 text-purple-600 cursor-pointer focus:ring-purple-500 border-secondary-300" value="public">
                                    </div>
                                    <label for="settings-option-0" class="ml-3 flex flex-col cursor-pointer">
                                        <span :class="{ 'text-purple-900': active === 0, 'text-secondary-900': !(active === 0) }" class="block text-sm font-medium text-purple-900">
                                            {{ __('Public access') }}
                                        </span>
                                        <span :class="{ 'text-purple-700': active === 0, 'text-secondary-500': !(active === 0) }" class="block text-sm text-purple-700">
                                            {{ __('This event would be available to anyone who has need to come') }}
                                        </span>
                                    </label>
                                </div>

                                <div :class="{ 'border-secondary-200': !(active === 1), 'bg-purple-50 border-purple-200 z-10': active === 1 }" class="relative border p-4 flex border-secondary-200">
                                    <div class="flex items-center h-5">
                                        <input wire:model="privacy" id="settings-option-1" name="privacy_setting" type="radio" @click="select(1)" @keydown.space="select(1)" @keydown.arrow-up="onArrowUp(1)" @keydown.arrow-down="onArrowDown(1)" class="h-4 w-4 text-purple-600 cursor-pointer focus:ring-purple-500 border-secondary-300" value="private">
                                    </div>
                                    <label for="settings-option-1" class="ml-3 flex flex-col cursor-pointer">
                                        <span :class="{ 'text-purple-900': active === 1, 'text-secondary-900': !(active === 1) }" class="block text-sm font-medium text-secondary-900">
                                            {{ __('Private to Church Members') }}
                                        </span>
                                        <span :class="{ 'text-purple-700': active === 1, 'text-secondary-500': !(active === 1) }" class="block text-sm text-secondary-500">
                                            {{ __('Only members of the church would be able to access') }}
                                        </span>
                                    </label>
                                </div>

                                <div :class="{ 'border-secondary-200': !(active === 2), 'bg-purple-50 border-purple-200 z-10': active === 2 }" class="relative border rounded-bl-md rounded-br-md p-4 flex border-secondary-200">
                                    <div class="flex items-center h-5">
                                        <input wire:model="privacy" id="settings-option-2" name="privacy_setting" type="radio" @click="select(2)" @keydown.space="select(2)" @keydown.arrow-up="onArrowUp(2)" @keydown.arrow-down="onArrowDown(2)" class="h-4 w-4 text-purple-600 cursor-pointer focus:ring-purple-500 border-secondary-300" value="invitation">
                                    </div>
                                    <label for="settings-option-2" class="ml-3 flex flex-col cursor-pointer">
                                        <span :class="{ 'text-purple-900': active === 2, 'text-secondary-900': !(active === 2) }" class="block text-sm font-medium text-secondary-900">
                                            {{ __('By invitation') }}
                                        </span>
                                        <span :class="{ 'text-purple-700': active === 2, 'text-secondary-500': !(active === 2) }" class="block text-sm text-secondary-500">
