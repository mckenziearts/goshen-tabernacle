<div>
    ...
    <div class="hidden mt-8 sm:block">
        <div class="align-middle inline-block min-w-full border-b border-secondary-200">
            <table class="min-w-full">
                <thead>
                    <tr class="border-t border-secondary-200">
                        ...
                        <th class="hidden md:table-cell px-6 py-3 border-b border-secondary-200 bg-secondary-50 text-right text-xs font-medium text-secondary-500 uppercase tracking-wider">
                            {{ __('Last updated') }}
                        </th>
                ...
                <tbody class="bg-white divide-y divide-secondary-100" x-max="1">
                    @forelse($events as $event)
                        <tr>
                                    ...
                                    <div class="flex items-center">
                                        ...
                                        @else
                                            <div class="bg-secondary-200 flex items-center justify-center h-8 w-8 rounded">
                                                <svg class="w-5 h-5 text-secondary-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <td class="hidden md:table-cell px-6 py-3 whitespace-nowrap text-sm text-secondary-500 text-right capitalize">
                            ...
                            <td class="pr-6">
                                <div x-data="{ open: false }" @keydown.escape="open = false" @click.away="open = false" class="relative flex justify-end items-center">
                                    <button id="project-options-menu-0" aria-haspopup="true" :aria-expanded="open" type="button" @click="open = !open" class="w-8 h-8 bg-white inline-flex items-center justify-center text-secondary-400 rounded-full hover:text-secondary-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-rose-500">
                                        <span class="sr-only">{{ __('Open options') }}</span>
                                        <x-heroicon-s-dots-vertical class="w-5 h-5" />
                                    </button>
                                    <div x-show="open" x-description="Dropdown panel, show/hide based on dropdown state." x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class="mx-3 origin-top-right absolute right-7 top-0 w-48 mt-1 rounded-md shadow-lg z-10 bg-white ring-1 ring-black ring-opacity-5 divide-y divide-secondary-200" role="menu" aria-orientation="vertical" aria-labelledby="project-options-menu-0" style="display: none;">
                                        <div class="py-1" role="none">
                                            <a href="{{ route('cp.events.show', $event) }}" class="group flex items-center px-4 py-2 text-sm text-secondary-700 hover:bg-secondary-100 hover:text-secondary-900" role="menuitem">
                                                <x-heroicon-s-eye class="mr-3 h-5 w-5 text-secondary-400 group-hover:text-secondary-500" />
                                                {{ __('Afficher') }}
                                            </a>
                                            <a href="{{ route('cp.events.edit', $event) }}" class="group flex items-center px-4 py-2 text-sm text-secondary-700 hover:bg-secondary-100 hover:text-secondary-900" role="menuitem">
                                                <x-heroicon-s-pencil-alt class="mr-3 h-5 w-5 text-secondary-400 group-hover:text-secondary-500" />
                                                {{ __('Editer') }}
                                            </a>
                                        </div>
                                        <div class="py-1" role="none">
                                            <button wire:click="remove({{ $event->id }})" type="button" class="w-full group flex items-center px-4 py-2 text-sm text-secondary-700 hover:bg-red-100 hover:text-red-800 focus:outline-none" role="menuitem">
                                                <x-heroicon-s-trash class="mr-3 h-5 w-5 text-secondary-400 group-hover:text-red-800" />
                                                {{ __('Supprimer') }}
                                            </button>
                                        </div>
                                    </div>
